...
///
EFI_DRIVER_BINDING_PROTOCOL gVulnGetVarDriverDriverBinding = {
  VulnGetVarDriverDriverBindingSupported,
  VulnGetVarDriverDriverBindingStart,
  VulnGetVarDriverDriverBindingStop,
  DRIVER_TEMPLATE_VERSION,
  NULL,
  NULL
};
  ...
  )
{
  ...
  Status = gBS->UninstallMultipleProtocolInterfaces (
                  ImageHandle,
                  &gEfiDriverBindingProtocolGuid, &gVulnGetVarDriverDriverBinding,
                  NULL
                  );
  if (EFI_ERROR (Status)) {
    return Status;
...
VOID
EFIAPI
VulnGetVarDriverCallbackFunction (
 IN EFI_EVENT        Event,
 IN VOID             *Context
 )
{
	if (runOnce==0)
	   {
		 ...
		 PrintMain( gImageHandle,gSystemTable);
...
EFI_STATUS
EFIAPI
VulnGetVarDriverDriverEntryPoint (
  IN EFI_HANDLE        ImageHandle,
  IN EFI_SYSTEM_TABLE  *SystemTable
  )
{
  ...
  Status = EfiLibInstallDriverBinding (
             ImageHandle,
             SystemTable,
             &gVulnGetVarDriverDriverBinding,
             ImageHandle
             );
